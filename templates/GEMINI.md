...
**자연어 수정 요청에도 동일하게 적용된다.** 코드 수정 요청이 오면 응답하기 전에 반드시 `constitution.md`와 `designs/design-tokens.css`를 **직접 열어서** 내용을 확인한 후 작업하라. UI 수정이라면 해당하는 토큰(`var(--토큰명)`)을 사용하고, 토큰에 없는 값이라면 토큰을 추가한 후 사용한다. 어떤 경우에도 헌법 위반이나 CSS 하드코딩을 하지 않는다.
