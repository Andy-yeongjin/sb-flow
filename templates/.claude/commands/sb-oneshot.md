...
3. **입력 자산 감지**: 프로젝트 루트에서 아래 파일을 탐색하여 기록합니다:
   ...
   | 자산 | 탐색 패턴 | 용도 |
   |------|----------|------|
   | **PRD** | `*prd*.md`, `*PRD*.md` (대소문자 무관, 파일명에 "prd" 포함) | 기능 요구사항 원본 — spec.md 작성 시 모든 요구사항을 빠짐없이 반영 |
   | **디자인** | `*.pen` | UI 시각적 원본 — spec.md의 유저 시나리오·화면 목록에 반영 (헌법 제17조) |

   - 감지된 파일이 있으면 → 경로를 변수에 기록하고 보고:
     ```
     [0/12] 입력 자산 감지 완료
       PRD: prd.md ✅
       디자인: design.pen ✅
     ```
   - 감지된 파일이 없으면 → `[0/12] 추가 입력 자산 없음 — 텍스트 설명 기반으로 진행` 후 계속
...
(헌법 제17조: .pen 파일은 UI 구현의 절대적 최우선 시각적 원본)
