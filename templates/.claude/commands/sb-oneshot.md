...
3. **입력 자산 감지**: 프로젝트 루트에서 아래 파일을 탐색하여 기록합니다:
   ...
   | 자산 | 탐색 패턴 | 필수 | 용도 |
   |------|----------|------|------|
   | **디자인 헌법** | `design-constitution.md` | **필수** | 정부 UIUX 가이드라인 — WCAG 2.1 AA, 터치 타깃 44px, 색상 대비 4.5:1 등 UI 품질의 불변 기준 (헌법 제16·17조) |
   ...
   - **디자인 헌법 검증** (필수):
     - `design-constitution.md`가 **존재**해야 진행 가능
     - 없으면 → **즉시 중단** 후 안내:
       ```
       ⚠️  design-constitution.md 파일이 없습니다.
...
design-constitution.md를 참조해서 정부 UIUX 가이드라인을 준수해줘: 색상 대비 4.5:1 이상(WCAG 2.1 AA), 터치 타깃 최소 44px, 시맨틱 HTML 마크업, 키보드 접근성(aria 속성, focus 관리), 한국어 UI 문구 표준 적용.
```
...
### [11/12] 갭 분석 + 디자인 헌법 검증 — `/pdca analyze`
...
`design-constitution.md`를 참조해서 구현된 UI 컴포넌트를 아래 기준으로 검증합니다:
...
선택 A (권장): 즉시 수정 후 재검증
선택 B: 이 상태로 완료하고 나중에 /sb-design 으로 재검증
```

갭 분석 Match Rate 결과에 따라:
- **Match Rate ≥ 90% + 헌법 위반 없음** → `✅ 갭 분석 + 디자인 헌법 검증 통과` 후 완료 보고로 진행
