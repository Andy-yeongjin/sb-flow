...
**코드 수정 요청이 오면 응답하기 전에 반드시 `constitution.md`, `designs/design.md`, `designs/design-tokens.css`를 Read 도구로 직접 열어서 확인한 후 작업하라.** UI 수정이라면 해당하는 토큰(`var(--토큰명)`)을 사용하고, 토큰에 없는 값이라면 토큰을 추가한 후 사용한다. 위반이 하나라도 있으면 즉시 수정한다. 검증 없이 "완료"라고 말하는 것은 허용되지 않는다.
