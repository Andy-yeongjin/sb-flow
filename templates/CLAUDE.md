...
> bkit PDCA로 개발하며, 헌법과 디자인 헌법을 모든 작업의 최우선 기준으로 삼습니다.
...
**코드든 문서든, 결과물을 최종 제출하기 전에 반드시 `constitution.md`와 `designs/design-constitution.md`를 다시 읽고 위반 항목을 스스로 점검·수정한 후 완료 처리하라. 위반이 하나라도 있으면 즉시 수정한다. 검증 없이 "완료"라고 말하는 것은 허용되지 않는다.**

**자연어 수정 요청에도 동일하게 적용된다.** 코드 수정 요청이 오면 응답하기 전에 반드시 `constitution.md`와 `designs/design.md`, `designs/design-tokens.css`를 **Read 도구로 직접 열어서** 내용을 확인한 후 작업하라. UI 수정이라면 해당하는 토큰(`var(--토큰명)`)을 사용하고, 토큰에 없는 값이라면 토큰을 추가한 후 사용한다. 어떤 경우에도 헌법 위반이나 CSS 하드코딩을 하지 않는다.
...
`/pdca plan`, `/pdca design`, `/pdca do` 실행 시 `@vercel-react-best-practices`를 함께 사용합니다.
