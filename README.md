...
#### (선택) 디자인 파일이 있다면 먼저 실행

Pencil.dev(`.pen`) 또는 Stitch(ZIP)로 디자인을 만든 경우, `/sb-oneshot` 전에 아래를 먼저 실행하세요:

- **Pencil.dev**: `.pen` 파일을 `designs/` 폴더에 넣기
- **Stitch**: ZIP 압축 해제 후 `designs/stitch/` 폴더에 넣기

```
/sb-design
```

디자인 소스를 분석해서 색상·폰트·간격 등 디자인 토큰을 자동 추출하고, 정부 UIUX 가이드라인(design-constitution.md) 기준으로 자동 교정 후 `designs/design.md`와 `designs/design-tokens.css`를 생성합니다.
...
| `designs/design.md` | 프로젝트 디자인 시스템 명세 (.pen 또는 Stitch에서 생성) |
...
| 제17조 | 디자인 원본 준수 — .pen 또는 Stitch는 구조/배치 원본, 시각적 수치는 design.md 토큰으로 교정 |
