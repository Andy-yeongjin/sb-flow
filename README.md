...
Pencil.dev에서 만든 `.pen` 파일을 프로젝트 루트에 넣은 경우, `/sb-oneshot` 전에 아래를 먼저 실행하세요:

```
/sb-design
```

`.pen` 파일을 분석해서 색상·폰트·간격 등 디자인 토큰을 자동 추출하고 `design.md`와 `design-tokens.css`를 생성합니다. 이 과정에서 `design-constitution.md`(정부 UIUX 가이드라인)를 기준으로 색상 대비·터치 타깃 등을 자동 검증·교정합니다.

> **팀 프로젝트**: 리드가 1회 실행 후 `design.md`·`design-tokens.css`를 git에 커밋하면 팀원들은 따로 실행할 필요 없습니다.
...
> **PRD 파일 자동 감지**: 프로젝트 루트에 PRD 파일(`*prd*.md`)이 있으면 자동으로 감지하여 명세서 작성에 반영합니다.
