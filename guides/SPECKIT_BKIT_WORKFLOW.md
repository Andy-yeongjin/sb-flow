...
| **bkit PDCA** | 8~16.5단계 (구현·검증·보고 전담) | 개발 계획 → 구현 → 갭 분석 → 개선 → 브라우저 검증 → 보고 | "**어떻게** 만들고 **제대로** 됐는가?" |
| **sb-flow** | 0단계(준비) + 17단계(동기화) | `/sb-design` 디자인 추출 → `/sb-bridge` 브릿지 생성 → `/sb-sync` 문서 동기화 | "설계·구현·문서를 항상 일치시킨다" |

> **`/sb-oneshot`** — 위 전체 파이프라인(1~17단계)을 명령어 하나로 자동 완주합니다. PRD 파일(`.md`)과 `.pen` 디자인 파일을 자동 감지하여 반영합니다.
...
모든 설치가 끝나면 Claude 데스크톱 앱을 재시작합니다.

> CLI(`claude --dangerously-skip-permissions`)를 사용하는 경우 `Ctrl+C`로 종료 후 다시 실행해야 새 명령어가 인식됩니다.
...
flowchart TD
    START(["🆕 새 프로젝트 시작"])
    ONESHOT(["⚡ /sb-oneshot [기능 설명]<br/>전체 파이프라인 자동 완주"])

    subgraph P["🎨 준비 — sb-flow (선택)"]
        direction TB
        SD["/sb-design<br/>.pen → design.md<br/>+ design-tokens.css<br/>(design-constitution 자동 검증)"]
    end
    ...
    subgraph A["🅰 SpecKit — 설계 구간"]
        ...
        S75[["/sb-bridge<br/>SPEC_CONTEXT.md 브릿지 문서 생성"]]
        S1 --> S2 --> S3 --> S4 --> S75
    end
    ...
    subgraph C["🅲 bkit — 검증·보고 구간"]
        ...
        S14[["브라우저 검증<br/>Playwright / browser_subagent<br/>design-constitution 준수 확인"]]
    ...
    subgraph D["🅳 동기화 — sb-flow"]
        S17[["/sb-sync<br/>문서 사후 동기화<br/>SPEC_CONTEXT.md 갱신"]]
    end
    ...
    style ONESHOT fill:#fff9c4,stroke:#F9A825,stroke-width:2px,stroke-dasharray:5 5
    style P fill:#fce4ec,stroke:#E91E63,stroke-width:1px,stroke-dasharray:4 4
...
| **명령어** | `/sb-setup` 또는 `SPEC_CONTEXT.md` 읽기 요청 → `/pdca status` |
| **프레임워크** | sb-flow (`/sb-setup`) + bkit (`/pdca status`) |
...
가장 간단한 방법 — `/sb-setup`이 세션 재개에 필요한 컨텍스트를 자동으로 복원합니다:
...
| ✨ 새 기능 — **소·중규모** | **대화형 개발** | `start.bat` (또는 `npm run dev`) 후 자연어로 요청 |
...
| 규모 | 해당하는 경우 | 접근 방식 |
|------|------------|----------|
| **소규모** | 단일 컴포넌트, 작은 로직 추가 | `start.bat` (또는 `npm run dev`) + 대화형 → 완료 후 SPEC_CONTEXT.md 업데이트 |
| **중규모** | 여러 파일, UI 흐름 변경 | `start.bat` (또는 `npm run dev`) + 대화형 → 10.5단계 → SPEC_CONTEXT.md 업데이트 |
...
### ⚡ 자동 완주 단축키 — `/sb-oneshot`

> 아래 1~17단계를 명령어 하나로 자동 완주합니다
...
```
[설계] constitution → specify → plan → tasks
→ [브릿지] /sb-bridge (SPEC_CONTEXT.md 생성)
→ [구현] /pdca plan → /pdca design → /pdca do
→ [검증] /pdca analyze → /pdca iterate (< 90%)
→ [브라우저 검증] Playwright / browser_subagent
→ [동기화] /sb-sync
```

**자동 감지**:
- `prd/` 폴더의 `.md` 파일 → 명세서 작성에 자동 반영
- `designs/` 폴더의 `.pen` 파일 → `/sb-design` 실행 후 design-tokens 반영

> 개별 단계를 직접 제어하고 싶다면 아래의 단계별 가이드를 따릅니다.

---

### 🎨 준비 단계 — `/sb-design` *(선택, .pen 파일이 있을 때)*

#### 0단계. 디자인 추출 — `/sb-design`

| 항목 | 내용 |
|------|------|
| **목적** | Pencil.dev `.pen` 파일에서 색상·폰트·간격 등 디자인 토큰 추출 및 검증 |
| **명령어** | `/sb-design` |
| **프레임워크** | sb-flow |
| **실행 시점** | `.pen` 파일이 있을 때, 1단계 실행 전 (선택) |

**상세 설명**

`designs/` 폴더에 `.pen` 파일이 있는 경우, 설계를 시작하기 전에 디자인 토큰을 먼저 추출합니다:

1. `.pen` 파일 분석 → 색상·폰트·간격·그림자 등 디자인 토큰 추출
2. `designs/design-constitution.md`(정부 KRDS 가이드라인)를 기준으로 자동 검증·교정:
   - 색상 대비 4.5:1 이상 확보
   - 터치 타깃 44px 이상
   - 키보드 접근성 준수
3. 교정된 토큰으로 `designs/design.md`와 `designs/design-tokens.css` 생성

```
/sb-design
```

**입력**: `designs/*.pen` + `designs/design-constitution.md`

**산출물**:
- `designs/design.md` — 프로젝트 디자인 시스템 명세
- `designs/design-tokens.css` — CSS 커스텀 프로퍼티 변수 파일

**주의사항**:
- `.pen` 파일이 없으면 실행하지 않습니다
- 팀 프로젝트: 리드가 1회 실행 후 `designs/` 폴더를 git에 커밋하면 팀원들은 따로 실행할 필요 없습니다
- `/sb-oneshot` 실행 시 `.pen` 파일이 감지되면 자동으로 이 단계가 먼저 실행됩니다
...
```
/sb-bridge specs/001-pomodoro-timer
```

`spec-dir`을 생략하면 `specs/` 디렉토리에서 가장 최근 생성된 디렉토리를 자동으로 감지합니다:

```
/sb-bridge
```

`/sb-bridge`가 `specs/NNN-기능명/` 디렉토리의 모든 산출물을 읽고 `SPEC_CONTEXT.md`를 자동으로 채워줍니다.
...
#### 14단계. 브라우저 검증 — Playwright / browser_subagent

| 항목 | 내용 |
|------|------|
| **목적** | 실제 브라우저에서 기능·디자인·접근성을 자동 검증하고 자체 수정 |
| **명령어** | (자동 실행 — `/sb-oneshot` 내부에서 자동 트리거) |
| **프레임워크** | sb-flow (Claude: Playwright MCP / Gemini: browser_subagent) |
| **실행 시점** | Match Rate >= 90% 달성 후 |

**상세 설명**

갭 분석에서 90% 이상 달성하면 브라우저 검증이 자동으로 실행됩니다:

1. **기능 검증**: 구현된 기능이 실제 브라우저에서 spec.md의 시나리오대로 동작하는지 확인
2. **디자인 검증**: `designs/design-tokens.css` 기준으로 색상·폰트·간격 일치 여부 확인
3. **접근성 검증**: `designs/design-constitution.md` 기준으로 색상 대비(4.5:1), 터치 타깃(44px) 등 자동 확인
4. **자체 수정**: 발견된 문제를 즉시 자동 수정

**AI별 도구 차이**:

| AI 에이전트 | 브라우저 도구 |
|------------|-------------|
| Claude Code | Playwright MCP (`mcp__playwright__*`) |
| Gemini CLI | `browser_subagent` |

**입력**: 실행 중인 애플리케이션 (`start.bat` 또는 `npm run dev` 상태) + spec.md 시나리오

**산출물**:
- 브라우저 검증 결과 리포트
- 자동 수정된 코드 (디자인·접근성 이슈)

**주의사항**:
- 검증 전 개발 서버가 실행 중이어야 합니다 (`start.bat` 실행)
- design-constitution.md 위반이 발견되면 자동으로 교정합니다
- `/sb-oneshot` 실행 시 이 단계가 자동으로 포함됩니다

#### 14.5단계. 로그 기반 QA — `/zero-script-qa` *(선택, Docker 환경)*

| 항목 | 내용 |
|------|------|
| **목적** | Docker 로그를 실시간 모니터링하여 API 요청/응답 이슈 자동 감지 |
| **명령어** | `/zero-script-qa` |
| **프레임워크** | bkit 스킬 |
| **실행 시점** | 브라우저 검증(14단계) 후, Docker 환경에서만 |
| **사용 에이전트** | `qa-monitor` |

> Docker 없는 프로젝트(Next.js dev server 등)는 `start.bat` 터미널 로그를 직접 확인하며 수동 테스트합니다. 이 단계는 `docker-compose.yml`이 있는 경우에만 유효합니다.
...
자동으로 처리되는 항목:
- bkit plan/design 문서 동기화 (로직·UX 변경 시)
- `specs/NNN-기능명/tasks.md` — `[ ]` → `[x]`, 새 태스크 추가, deprecated 처리
- `specs/NNN-기능명/spec.md` — FR 수정·추가·deprecated
- `SPEC_CONTEXT.md` — 최종 상태 갱신
- `/speckit.analyze` — SpecKit 문서 간 일관성 검증 (자동 실행)

**산출물**:
- `specs/NNN-기능명/spec.md` — FR 전체 현실 반영
- `specs/NNN-기능명/tasks.md` — 완료 체크박스 + 새 태스크
- `SPEC_CONTEXT.md` — 최종 상태 갱신
...
|---|------|--------|--------|-----------|--------|
| ⚡ | 단축키 | **전체 자동 완주** | `/sb-oneshot [기능 설명]` | — | 전체 파이프라인 자동 실행 |
| 0 | 🔁 재개 | **세션 재개 & 컨텍스트 회복** | `/sb-setup` → `/pdca status` | **기존 프로젝트 시 항상** | 작업 방향 결정 |
| 0 | 🎨 준비 | **디자인 추출** | `/sb-design` | 선택 (.pen 파일 있을 때) | design.md + design-tokens.css |
...
| 7.5 | 🅰→🅱 | **브릿지 문서 작성** | `/sb-bridge [spec-dir]` | **필수** | **SPEC_CONTEXT.md** |
...
| 14 | 🅲 검증 | **브라우저 검증** | 자동 (Playwright / browser_subagent) | **필수** | 검증 리포트 + 자동 수정 |
| 14.5 | 🅲 검증 | 로그 기반 QA | `/zero-script-qa` | 선택 (Docker 환경) | QA 리포트 |
...
| 17 | 🅳 동기화 | **문서 사후 동기화** | `/sb-sync` | **필수** | tasks.md [x] + spec.md 갱신 + SPEC_CONTEXT.md 갱신 |

---

## 빠른 참조 — 최소 실행 경로

### 방법 1: `/sb-oneshot`으로 전체 자동 완주 (권장)

```
/sb-oneshot [기능 설명]
```

설계~브라우저 검증~동기화까지 14단계를 자동 완주합니다. `prd/` 폴더의 PRD 파일과 `designs/`의 `.pen` 파일을 자동 감지하여 반영합니다.
...
→ /sb-bridge → /pdca plan → /pdca design → 구현
→ /pdca analyze → [브라우저 검증] → /pdca report → /sb-sync
```

11단계로 축소된 최소 경로이며, 나머지는 품질을 높이기 위한 선택/권장 단계입니다.

---

## FAQ

**Q: `/sb-oneshot`과 수동 단계별 실행은 어떻게 다른가요?**

A: `/sb-oneshot`은 1~17단계를 자동으로 연결하여 완주합니다. 명령어 하나로 설계·구현·검증·동기화가 모두 완료됩니다. 단계별 수동 실행은 각 단계를 직접 제어하고 싶거나, 특정 단계에서 설계를 검토·수정하고 싶을 때 사용합니다. 처음에는 `/sb-oneshot`으로 전체 흐름을 경험한 후, 필요에 따라 수동 실행으로 전환하는 것을 권장합니다.

**Q: `/sb-design`은 언제 실행해야 하나요?**

A: Pencil.dev에서 만든 `.pen` 디자인 파일이 있을 때만 실행합니다. `designs/` 폴더에 `.pen` 파일을 넣고 `/sb-design`을 실행하면 색상·폰트·간격 등 디자인 토큰이 자동 추출되어 `designs/design.md`와 `designs/design-tokens.css`가 생성됩니다. 팀 프로젝트에서는 리드가 1회 실행 후 git에 커밋하면 됩니다. `/sb-oneshot`도 `.pen` 파일을 자동 감지하여 이 단계를 포함합니다.

**Q: `/sb-bridge`와 직접 SPEC_CONTEXT.md를 작성하는 것은 어떻게 다른가요?**

A: 이전에는 Claude에게 직접 "specs/ 디렉토리를 읽고 SPEC_CONTEXT.md를 채워줘"라고 요청했지만, `/sb-bridge`가 이 과정을 자동화합니다. `spec-dir`을 인자로 전달하거나 생략하면 가장 최근 specs 디렉토리를 자동 감지합니다. 결과물은 동일합니다.
...
- **소·중규모**: `start.bat` (또는 `npm run dev`) + 대화형 → 완료 후 SPEC_CONTEXT.md 업데이트
...
아래 네 가지를 모두 처리하고 구현 코드와 비교 후  spec.md, plan.md, tasks.md를 업데이트:
